import { 
  Box, TextField, IconButton, Button, Tooltip, Stack, Typography, CircularProgress, Popover, Paper, InputAdornment, Grid,
  Dialog, DialogContent, useTheme, useMediaQuery
} from "@mui/material";
...
const RutinasForm = ({ anchorEl, open, data, onChange, onClose, onSubmit, loading, videoFile, setVideoFile, removeVideo, setRemoveVideo }) => {
    ...
    const content = (
        <Paper sx={{ p: isMobile ? 2 : 4, width: isMobile ? '95vw' : 400 }}>
                ...
                <Grid container spacing={2.5}>
                    ...
                    <Grid item xs={12}>
                        <TextField
                            ...
                            onChange={(e) => onChange({ ...data, ejercicio: e.target.value })}
                            required
                            disabled={loading}
                            sx={fieldSx}
                            slotProps={{
                    ...
                    <Grid item xs={4}>
                        <TextField
                            ...
                            onChange={(e) => onChange({ ...data, repeticiones: Number(e.target.value) })}
                            required
                            disabled={loading}
                            sx={fieldSx}
                            slotProps={{
                    ...
                    <Grid item xs={4}>
                        <TextField
                            ...
                            onChange={(e) => onChange({ ...data, descanso: e.target.value })}
                            required
                            disabled={loading}
                            sx={fieldSx}
                            slotProps={{
                    ...
                    <Grid item xs={12}>
                        <Box display="flex" alignItems="center" gap={1}>
                            <Box display="flex" alignItems="center" gap={1}>
                                <Tooltip
                                    title={videoFile || (data?.demostracion && !removeVideo) ? "Reemplazar video" : "Agregar video"}
                                >
                                    <Box sx={{ position: "relative", width: 40, height: 40 }}>
                                        {(videoFile || (data?.demostracion && !removeVideo)) ? (
                                            <FindReplaceIcon
                                                onClick={() => document.getElementById("video-input").click()}
                                                sx={{ cursor: loading ? "not-allowed" : "pointer", color: loading ? "text.disabled" : "primary.main", fontSize: 40, position: "absolute", top: 0, left: 0 }}
                                            />
                                        ) : (
                                            <MovieIcon
                                                onClick={() => document.getElementById("video-input").click()}
                                        ...
                                        {loading && (
                                            <CircularProgress size={40} sx={{ color: "primary.main", position: "absolute", top: 0, left: 0, zIndex: 1 }} />
                                        )}
                                    </Box>
                                </Tooltip>

                                <Typography variant="body2">
                                    <strong>{removeVideo ? "Video eliminado" : videoFile ? videoFile.name : data?.demostracion ?? ""}</strong>
                                </Typography>

                                {(videoFile || data?.demostracion) && (
                                    <IconButton size="small" color="error" disabled={loading} onClick={() => {
                                        setVideoFile(null);
                                        setRemoveVideo(true);
                        ...
                        {isString && (
                            <Box mt={2}>
                                ...
                                >
                                    {isGif ? (
                                        <img src={demostracion} alt="Demostración" style={{ width: "100%", height: "100%", objectFit: "contain" }} />
                                    ) : (
                                        <video src={demostracion} autoPlay muted loop playsInline style={{ width: "100%", height: "100%", objectFit: "contain" }} />
                                    )}
                                </Box>
                            </Box>
                        )}
    ...
    // Render según tamaño de pantalla
    return isMobile ? (
        ...
        <Popover
            ...
            slotProps={{
                paper: { className: "scroll-hide", sx: { borderRadius: 3, boxShadow: "0px 12px 32px rgba(0,0,0,0.15)" } }
            }}
        >
            {content}
        </Popover>
