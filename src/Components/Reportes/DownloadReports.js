...
const DownloadReports = () => {
    ...
    const handleDownload = async (report) => {
        if (!report.allowedPlans.map(p => p.toUpperCase()).includes(plan.toUpperCase())) {
        ...
        try {
            ...
            const response = await report.serviceCall();
    ...
    return (
        <Box sx={{ backgroundColor: theme.palette.background.default, minHeight: "100vh" }}>
            ...
            <Grid container spacing={4} justifyContent="center">
                {reports.map((report) => {
                    const gradient = theme.palette.mode === "dark" ? report.gradientDark : report.gradientLight;
                    return (
